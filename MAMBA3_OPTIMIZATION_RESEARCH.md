...
`scripts/mamba3_find_16gb_train_target.sh` showed:
...
- `--deepspeed-config configs/deepspeed_zero2_cpu_offload.json` only after Mamba-3 TileLang/DeepSpeed compatibility is fixed.
- `--deepspeed-config configs/deepspeed_zero3_param_offload.json` only after ZeRO-2 compatibility is fixed and parameter memory is the bottleneck.
...
- `./neurova.sh` uses `scripts/mamba3_safe_chat.py`, a model-only full-forward deterministic path, because the latest speaking checkpoint gives better answers there than the recurrent/CUDA-graph path.
...
- `scripts/mamba3_decode_tune.py` is the canonical quality/speed sweep for deciding when a checkpoint is ready to switch back to recurrent/CUDA-graph decode.
...
- Run `./neurova.sh mamba3 tune` after every new speaking checkpoint and keep the best config evidence under `runs/.../decode_tune/`.
...
- Install and validate `bitsandbytes` and `deepspeed` through `scripts/mamba3_install_efficiency_deps.sh`.
- Run `scripts/mamba3_train_stability_ladder.sh` to compare SISO, MIMO r2, MIMO r2+attention, MIMO r4, and MIMO r4+attention on the same no-teacher curriculum.

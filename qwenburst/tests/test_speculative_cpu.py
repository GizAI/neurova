...
from qwenburst.speculation import (
    DraftRequest,
    SpeculativeBenchmarkResult,
    SpeculativeProbeResult,
)
from qwenburst.tuning import speculative_verifier_mode
...
def test_speculative_verifier_default_is_transaction_block(monkeypatch):
    monkeypatch.delenv("QWENBURST_SPECULATIVE_VERIFIER", raising=False)
    assert speculative_verifier_mode() == "transaction_block"


def test_speculative_verifier_env_override(monkeypatch):
    monkeypatch.setenv("QWENBURST_SPECULATIVE_VERIFIER", "sequential")
    assert speculative_verifier_mode() == "sequential"

...
MODEL_NAME="${MODEL_NAME:-qwenburst-qwen3.6-27b-q4-marlin}"
...
if ! ss -ltn | grep -q ":${SERVER_PORT} "; then
  nohup env \
    QWENBURST_REQUIRE_CUDA_EXT=1 \
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python -m qwenburst.server \
      --adapter qwen36 \
      --hf-model "$MODEL_DIR" \
      --qb-model "$QB_DIR" \
    ...
    > "${LOG_DIR}/qwenburst_server.log" 2>&1 &
  echo $! > "${LOG_DIR}/qwenburst_server.pid"
fi
...
if ! curl -fsS "http://127.0.0.1:${SERVER_PORT}/v1/models" >/dev/null; then
  echo "qwenburst server did not become ready; see ${LOG_DIR}/qwenburst_server.log" >&2
  exit 1
fi
...
if ! ss -ltn | grep -q ":${OPENWEBUI_COMPAT_PORT} "; then
  nohup socat \
    ...
    > "${LOG_DIR}/qwenburst_openwebui_proxy.log" 2>&1 &
  echo $! > "${LOG_DIR}/qwenburst_openwebui_proxy.pid"
fi

echo "QwenBurst: http://127.0.0.1:${SERVER_PORT}/v1"

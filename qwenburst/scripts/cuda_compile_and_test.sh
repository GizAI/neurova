...
#   2. compile qwenburst_cuda for SM89
#   3. run kernel parity tests
#   4. run microbenchmarks

cd "$(dirname "$0")/.."
export QWENBURST_REQUIRE_CUDA_EXT=1
export QWENBURST_CUDA_ARCH_LIST="${QWENBURST_CUDA_ARCH_LIST:-8.9}"

python -m qwenburst.doctor --require-cuda
python -m pip install -U pip wheel setuptools ninja pytest
python -m pip install -v --no-build-isolation -e .
python - <<'PY'
import qwenburst_cuda
print('qwenburst_cuda loaded:', qwenburst_cuda.__doc__)
PY
pytest -q \
...
python benchmarks/bench_kernels.py --iters "${QWENBURST_BENCH_ITERS:-1000}"

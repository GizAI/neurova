...
from .model import WeightResolver
    ...
    for i in range(cfg.num_layers):
        ...
        if cfg.layer_type(i) == "gdn":
            ...
            req.extend([
                ...
                (f"layer_{i}_attn_o", (f"{sa}.o_proj.weight",)),
            ])
    return req


def main() -> None:
    ap = argparse.ArgumentParser(description="Audit converted QwenBurst checkpoint key coverage")
    ...
    with open(args.qb_model / "qwenburst_index.json", "r", encoding="utf-8") as f:
        index = json.load(f)

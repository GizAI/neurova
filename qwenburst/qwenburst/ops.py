...
class CPUFallbackOps:
    ...
    It is deliberately slow.  The 100 tok/s path requires qwenburst_cuda.
    """
    ...
    def lowbit_marlin_gemm_out(
        ...
        raise RuntimeError("lowbit_marlin_gemm_out requires qwenburst_cuda")
...
@lru_cache(maxsize=1)
def cuda_ops():
    force_cpu = os.environ.get("QWENBURST_CPU_FALLBACK", "0") == "1" or os.environ.get("QWENBURST_SKIP_CUDA_EXT", "0") == "1"
    if not force_cpu:
        try:
            return importlib.import_module("qwenburst_cuda")
        except Exception as exc:  # pragma: no cover
            if os.environ.get("QWENBURST_REQUIRE_CUDA_EXT", "0") == "1":
                raise RuntimeError(
                    "qwenburst_cuda is not built. Build on the RTX 4080 box with:\n"
                    "  QWENBURST_REQUIRE_CUDA_EXT=1 pip install -v -e ."
                ) from exc

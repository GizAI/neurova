...
# ══════════════════════════════════════════════════════════════
# QUERY PLANNER — structured answer engine
# ══════════════════════════════════════════════════════════════

class QueryPlanner:
    """Structured answer engine that queries WorldModel by EventFrame types."""

    def __init__(self, model: WorldModel, coref: Coref):
        self.model = model
        self.coref = coref

    def answer(self, question: str) -> str:
        q = question.strip().rstrip("?").lower().strip()
        if not q:
        ...
        # Classify question
        qtype, target_entity = self._classify(q, pw)
        target_orig = target_entity or pw.get("subj", "")
        target_norm, resolved = self.coref.resolve(target_orig) if target_orig else ("", False)
        if not target_norm:
            target_norm = target_orig

        entity = self.model.get(target_norm) if target_norm else None
        ...
        route_map = {
            ...
            "DOES_DO": lambda: self._answer_does(q, pw, entity, target_norm),
            "WHEN": lambda: self._answer_when(q, pw, entity, target_norm),
        }
        handler = route_map.get(qtype)
        if handler:
            return handler() if callable(handler) else handler
        return self._answer_fallback(q, pw, entity, target_norm)

    def _classify(self, q: str, pw: dict):
        qw = pw.get("qw", "")
        
        # WHAT_BORDERS
        bm = re.match(r"what\s+borders?\s+(.+?)(?:\s+to\s+the\s+(" + DIR_RE + r"))?\s*$", q)
        if bm:
            return "WHAT_BORDERS", bm.group(1)
        
        # WHAT_IS_DIRECTION: "what is in the southeast of X"
        dm = re.match(
            r"what\s+is\s+(?:located\s+)?(?:in|on|at|near|to)\s+"
            r"(?:the\s+)?(" + DIR_RE + r")\s*(?::?of|part\s+of|region\s+of|)?\s*(.+)", q
        )
        if dm:
            return "WHAT_IS_DIRECTION", dm.group(2)
        dm2 = re.match(r"what\s+is\s+(" + DIR_RE + r")\s+of\s+(.+)", q)
        if dm2:
            return "WHAT_IS_DIRECTION", dm2.group(1)

        # WHERE
        wm = re.match(r"where\s+(?:is|are|was|were)\s+(.+)", q)
        if wm:
            return "WHERE_IS", wm.group(1)

        # WHO
        if qw == "who":
            target = q.replace("who ", "").replace("is ", "").replace("did ", "").replace("does ", "").strip()
        ...
        if qw == "what":
            sm = re.match(r"what\s+is\s+(.+?)\s+separated\s+from", q)
            if sm:
                return "WHAT_SEPARATED_FROM", sm.group(1)
            wi = re.match(r"what\s+(?:is|are|was|were)\s+(.+)", q)
            if wi:
                return "WHAT_IS", wi.group(1)
            return "WHAT_IS", ""

        # HOW
        if q.startswith("how"):
            target = q.replace("how ", "").replace("are ", "").replace("is ", "").strip()
            return "HOW", target

        # WHEN
        if qw == "when":
            target = q.replace("when ", "").replace("did ", "").replace("does ", "").strip()
            return "WHEN", target
    ...
    def _answer_where(self, target_norm, entity):
        if not entity:
            return "I don't know where it is."
        if entity.location:
            return f"It is in {entity.location}."
        if entity.part_of:
            ploc = self.model.resolve_location(entity.part_of)
            if ploc:
                return f"It is in {ploc}."
            return f"It is part of {entity.part_of}."
        return "I don't know where it is."

    def _answer_who(self, target_norm, entity, pw):
        if not entity:
            return "I don't know who that is."
    ...
    def _answer_is(self, question, q, pw, entity, target_norm):
        if not entity:
            for w in q.split():
                e = self.model.get(norm(w))
                if e:
                    entity = e
                    break
        if not entity:
        ...
        for k, v in entity.attributes.items():
            if obj and (obj in v.lower() or v.lower() in obj):
                return "yes"
        if entity.location and obj and (obj in entity.location or entity.location in obj):
            return "yes"
        for p in entity.parts:
            if obj and (obj in p or p in obj):
                return "yes"
        for b in entity.borders:
            if obj and (obj in b or b in obj):
                return "yes"
        for p in entity.properties:
            if p in q or p.replace("_", " ") in q:
                return "yes"
        if entity.location and entity.location in q:
            return "yes"
        if entity.divided_into and "divid" in q:
            return "yes"
        if entity.separated_from and "separated" in q:
            return "yes"
        return "no"

    def _answer_what(self, q, pw, entity, target_norm, question):
        sm = re.match(r"what\s+is\s+(.+?)\s+separated\s+from", q)
        if sm:
            t = norm(sm.group(1))
            e = self.model.get(t)
            if e and e.separated_from:
                return e.separated_from
            return "I don't know."
        ...
        if dm:
            ...
            e = self.model.get(t)
            if e:
                if d in e.direction_relations:
                    return f"The {d} of {t} is {e.direction_relations[d]}."
                if hasattr(e, 'separation_direction') and e.separation_direction == d and e.separated_from:
                    return f"The {d} of {t} is {e.separated_from}."
            for name, ee in self.model.entities.items():
                if norm(name) != t:
                    for dr, neighbor in ee.direction_relations.items():
                        if neighbor == t and dr == d:
                            return f"The {d} of {t} is {name}."
            return "I don't know."
        dm2 = re.match(r"what\s+is\s+(" + DIR_RE + r")\s+of\s+(.+)", q)
        if dm2:
            t = norm(dm2.group(1))
            for d in sorted(DIR_WORDS, key=len, reverse=True):
                if q.startswith(f"what is {d} of"):
                    for name, ee in self.model.entities.items():
                        if norm(name) != t:
                            for dr, neighbor in ee.direction_relations.items():
                                if neighbor == t and dr == d:
                                    return f"The {d} of {t} is {name}."
                    e = self.model.get(t)
                    if e and d in e.direction_relations:
                        return f"The {d} of {t} is {e.direction_relations[d]}."
                    break
            return "I don't know."
    ...
    def _answer_how(self, target_norm, entity):
        if not entity:
            for name, ee in self.model.entities.items():
                if ee.divided_into:
                    entity = ee
                    break
        if entity and entity.divided_into:
            parts = ", ".join(entity.divided_into)
            crit = f" {entity.division_criterion}" if entity.division_criterion else ""
            return f"It is divided into {parts}{crit}."
        return "I don't know."

    def _answer_what_borders(self, q, entity, target_norm):
        bm = re.match(r"what\s+borders?\s+(.+?)(?:\s+to\s+the\s+(" + DIR_RE + r"))?\s*$", q)
        if bm:
            t = norm(bm.group(1))
            d = norm(bm.group(2)) if bm.group(2) else ""
            e = self.model.get(t)
            if e:
                if d and d in e.direction_relations:
                    return e.direction_relations[d]
                if e.borders:
                    return ", ".join(e.borders)
        if entity and entity.borders:
            return ", ".join(entity.borders)
        return "I don't know."

    def _answer_does(self, q, pw, entity, target_norm):
        if not entity:
            return "I don't know."
        obj = pw.get("obj", "")
        for k, v in entity.attributes.items():
            if obj and obj in v.lower():
                return "yes"
        if entity.location and obj and obj in entity.location:
            return "yes"
        return "no"

    def _answer_when(self, q, pw, entity, target_norm):
        if not entity:
            return "I don't know."
        temporal = entity.attributes.get("temporal", "")
        if temporal:
            return f"It was in {temporal}."
        return "I don't know."

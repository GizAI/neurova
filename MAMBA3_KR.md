...
This directory is the Mamba-3-only experimental path for Neurova.
...
python -m mamba3_kr.cli smoke --mode mimo-r4-tiny --tokenizer llama31 --seq-len 256 --device cuda
python -m mamba3_kr.cli train-tiny --mode mimo-r4-tiny --tokenizer llama31 --seq-len 256 --steps 20 --device cuda --data data/english_bootstrap.txt
python -m mamba3_kr.cli diagnose-decode --mode mimo-r4-tiny --tokenizer llama31 --seq-len 128 --device cuda
python -m mamba3_kr.cli bench-decode --mode mimo-r4-tiny --tokenizer llama31 --seq-len 128 --device cuda --cuda-graph --max-new 64
python -m mamba3_kr.cli state-prefill --mode mimo-r4-tiny --tokenizer llama31 --seq-len 256 --device cuda
python -m mamba3_kr.cli verify-rlvr --rlvr-data data/rlvr_verifier_bootstrap.jsonl
```

## ml-dmc8

```bash
bash scripts/deploy_mamba3_ml_dmc8.sh
```
...
```bash
TOKENIZER=llama31 HF_TOKEN=... bash scripts/deploy_mamba3_ml_dmc8.sh
```

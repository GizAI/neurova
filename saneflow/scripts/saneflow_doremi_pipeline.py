...
def main() -> None:
    ...
    parser.add_argument("--recipe", default="saneflow/configs/saneflow_practical_pretrain_mix.json")
    parser.add_argument("--out", default="saneflow/runs/doremi_proxy_practical_v1")
    parser.add_argument("--reference", default="")
    parser.add_argument("--reference-out", default="saneflow/runs/doremi_reference_practical_v1")
    parser.add_argument("--reference-steps", type=int, default=300)
    parser.add_argument("--tokenizer-path", default="saneflow/tokenizers/saneflow_fineweb_edu_16k")
    ...
    if not (args.skip_proxy_if_ratios_exist and ratio_out.exists()):
        ...
        if not reference:
            ...
            if not reference_ckpt.exists():
                ref_cmd = [
                    sys.executable,
                    "saneflow/scripts/saneflow_train_doremi_proxy.py",
                    "--recipe",
                    args.recipe,
                    "--out",
                    args.reference_out,
    ...
    run([sys.executable, "saneflow/scripts/saneflow_build_practical_pretrain_mix.py", "--recipe", args.recipe])

...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("saneflow/runs/saneflow_data_audit/audit.json"))
    ...
    if not paths:
        roots = [Path("saneflow/data/corpus/sources"), Path("saneflow/data/corpus/mixes"), Path("saneflow/data/corpus/sft_sources")]
        for root in roots:
            if root.exists():

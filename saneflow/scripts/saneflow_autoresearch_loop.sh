...
LOG_DIR="saneflow/runs/saneflow_autoresearch"
mkdir -p "$LOG_DIR"
...
import json
data = json.load(open("saneflow/configs/saneflow_profiles.json", encoding="utf-8"))
print(data["paths"]["fineweb_valid"])
PY
)"
  if [[ -f "$ckpt" ]]; then
    "$PYTHON" saneflow/scripts/saneflow_quality_gate.py \
      --ckpt "$ckpt" \
      --out "$out" \
...
run_reasoning_gate() {
  ...
  local out="$2"
  if [[ -f "$ckpt" ]]; then
    "$PYTHON" saneflow/scripts/saneflow_reasoning_gate.py \
      --ckpt "$ckpt" \
      --out "$out" \
...
profile_running() {
  ...
  pgrep -af "saneflow/scripts/saneflow_train.py" | grep -F -- "--out ${out}" | grep -v grep >/dev/null
}
...
profile_out() {
  local profile="$1"
  "$PYTHON" saneflow/scripts/saneflow_run.py list | "$PYTHON" -c 'import json,sys; data=json.load(sys.stdin); print(data[sys.argv[1]]["out"])' "$profile"
}
...
dense_tokenizer_path() {
  "$PYTHON" saneflow/scripts/saneflow_run.py list | "$PYTHON" -c 'import json,sys; data=json.load(sys.stdin); print(data["dmc9-dense-0.3b-v1"]["tokenizer_path"])'
}
...
ensure_dense_tokenizer() {
  ...
  "$PYTHON" saneflow/scripts/saneflow_train_tokenizer.py \
    ...
    --out "$tok" \
    --input \
      saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/train.jsonl \
      saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/valid.jsonl
}

ensure_doremi_mix() {
  local ratios="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/doremi_ratios.json"
...
ensure_practical_mix() {
  local train="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/train.jsonl"
  local valid="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/valid.jsonl"
  if [[ "${FORCE_REBUILD_PRACTICAL_MIX:-0}" != "1" && -s "$train" && -s "$valid" ]]; then
    log_json "practical_mix_exists"
    return 0
  fi
  "$PYTHON" saneflow/scripts/saneflow_build_practical_pretrain_mix.py || true
}

ensure_practical_v2_en_mix() {
  local train="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/train.jsonl"
  local valid="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/valid.jsonl"
  if [[ -s "$train" && -s "$valid" ]]; then
    log_json "practical_v2_en_mix_exists"
    return 0
  fi
  if pgrep -af "saneflow/scripts/saneflow_prepare_pretrain_v2.py" | grep -v grep >/dev/null; then
...
ensure_doremi_mix_v2_en() {
  local ratios="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/doremi_ratios.json"
  local train="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/train.jsonl"
  local valid="saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/valid.jsonl"
  if [[ "${DOREMI_AUTO:-1}" != "1" ]]; then
...
start_profile_once() {
  ...
  "$PYTHON" saneflow/scripts/saneflow_run.py start "$profile"
}
...
run_dmc8_once() {
  ...
  "$PYTHON" saneflow/scripts/saneflow_build_speak_pretrain_v1.py || true
  "$PYTHON" saneflow/scripts/saneflow_build_chatml_sft.py || true
  ...
  if gate_passed "$gate_path"; then
    ...
    mkdir -p saneflow/runs/saneflow_current
    ln -sfn "$(pwd)/${chat_model}" saneflow/runs/saneflow_current/model.pt
...
run_dmc9_once() {
  ...
  if [[ saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/doremi_ratios.json -nt saneflow/data/corpus/mixes/saneflow_practical_pretrain_v1/train.jsonl ]]; then
    "$PYTHON" saneflow/scripts/saneflow_build_practical_pretrain_mix.py || true
  fi
  ...
  if [[ -s saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/train.jsonl && -s saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/valid.jsonl && -f saneflow/data/corpus/mixes/saneflow_practical_pretrain_v2_en/doremi_ratios.json ]]; then

...
```text
model: SaneFlowLM
dense tokenizer: tokenizers/neurova_spm_unigram_64k
legacy recurrent tokenizer: tokenizers/saneflow_fineweb_edu_16k
profile registry: configs/saneflow_profiles.json
active program: configs/saneflow_research_program.json
...
profiles live in `configs/saneflow_profiles.json` and may point only at
...
4. Write data/corpus/mixes/saneflow_practical_pretrain_v1/doremi_ratios.json.
...
python scripts/saneflow_train_tokenizer.py \
  ...
  --out tokenizers/neurova_spm_unigram_64k \
  --input data/corpus/mixes/saneflow_practical_pretrain_v1/train.jsonl \
...
Before promotion to `runs/saneflow_current/model.pt`, a checkpoint must pass:

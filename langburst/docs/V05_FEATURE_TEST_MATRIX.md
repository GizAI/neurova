# LangBurst v0.5 Feature Test Matrix
...
LANGBURST_SKIP_CUDA_EXT=1 python -m pytest -q \
  langburst/tests/test_state_streaming_cpu.py \
  langburst/tests/test_memory_ttt_cpu.py \
  langburst/tests/test_quant_lowbit_cpu.py \
  langburst/tests/test_gdn_reference_cpu.py \
  langburst/tests/test_v04_correctness_cpu.py \
  langburst/tests/test_v05_runtime_cpu.py \
...
- CUDA Graph state reuse is explicitly skipped until langburst has a real graph
  decode path; no fake graph test is counted as coverage. `langburst-qwen-graph-audit`
...
```text
langburst-qwen3.6-27b-q4-marlin
```

...
LangBurst started as a narrow Qwen3.6-27B text runtime for one 16GB Ada GPU.
...
langburst target path
  ...
  - langburst-server OpenAI-compatible API
```
...
AdmissionController owns request admission, queue limits, and counters
...
adapter registry
  ...
  -> RuntimePlan
  -> EngineManager / AdmissionController
  -> RuntimeEngine
...
FastAPI request
  ...
  -> AdmissionController admission lease
...
complete. It can be tested with `LANGBURST_FAST_RAW_BLOCK=1`:
...
speculative decoding:
  ...
  Qwen3.6 Native MTP/NEXTN is enabled in the stateful default path with
  adaptive fallback to target-only decode.
  EAGLE/Medusa must implement the same proposer contract and pass measured
  speed gates before serving.
```

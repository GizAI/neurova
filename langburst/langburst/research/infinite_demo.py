...
def main() -> None:
    ap = argparse.ArgumentParser(description="LangBurst infinite-streaming state demo without loading 27B weights")
    ap.add_argument("--out", type=Path, default=Path("/tmp/langburst_infinite_demo"))
    ...
    mem = EpisodicMemory()
    mem.add_text("LangBurst infinite mode keeps recent KV and compressed GDN state.", record_id="design")

...
def quantize_symmetric_lowbit(w: torch.Tensor, group_size: int = 128, bits: int = 4) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    block_rows = int(os.environ.get("LANGBURST_QUANT_BLOCK_ROWS", "256"))
...
def convert(
    ...
    *,
    ...
    index: dict[str, dict] = {
        "format": f"langburst-q{bits}-{layout}-v1" if layout != "rowwise" else f"langburst-q{bits}-v4",
    ...
    with open(out_dir / "langburst_index.json", "w", encoding="utf-8") as f:
        json.dump(index, f, indent=2, ensure_ascii=False)


def main() -> None:
    ap = argparse.ArgumentParser(description="Convert HF safetensors to LangBurst low-bit format")

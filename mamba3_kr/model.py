...
class TinyTransformerLM(nn.Module):
    def __init__(self, cfg: Mamba3KRConfig, device: str, dtype: torch.dtype) -> None:
        super().__init__()
        self.config = cfg
...
def build_model(cfg: Mamba3KRConfig, device: str, dtype: torch.dtype) -> MambaLMHeadModel:
...
def preset_config(mode: str = "mimo-r4-tiny", vocab_size: int = 259) -> Mamba3KRConfig:
    if mode == "transformer-tiny":
        return Mamba3KRConfig(
    ...
    if mode == "siso":
        return Mamba3KRConfig(vocab_size=vocab_size)
    raise ValueError(f"unknown mode: {mode}")


def tiny_config(mode: str = "siso", vocab_size: int = 259) -> Mamba3KRConfig:
    return preset_config(mode, vocab_size)


def estimate_parameters(cfg: Mamba3KRConfig) -> int:
...
def effective_mlp_hidden_dim(cfg: Mamba3KRConfig) -> int:

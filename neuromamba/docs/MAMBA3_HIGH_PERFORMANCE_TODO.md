...
for mode in mimo-r4-paper-180m mimo-r4-440m mimo-r4-880m mimo-r4-1.5b; do
  python -m neuromamba.cli check-contract --mode "$mode" --tokenizer llama31
done
```
...
neuromamba/scripts/mamba3_train_governed_tiny.sh
...
```bash
neuromamba/scripts/mamba3_corpus_manifest.py neuromamba/data/*.txt neuromamba/data/*.jsonl
```
...
```bash
# Download enabled governed shards from neuromamba/configs/mamba3_corpus_sources.json.
MAX_DOCS=2000 MAX_BYTES=100000000 neuromamba/scripts/mamba3_prepare_corpora.sh

# Staged training: base pretrain -> validation -> instruction SFT -> eval.
BASE_STEPS=200 SFT_STEPS=200 neuromamba/scripts/mamba3_train_scientific_tiny.sh
```

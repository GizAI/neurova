...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NeuroMamba experimental runtime")
    ...
    for name in ("model-info", "check-contract", "smoke", "probe-kernel", "train-tiny", "train-packed", "train-answer", "train-multitask", "eval-loss", "eval-answer-loss", "generate", "fast-generate", "eval-english", "quality-gate", "diagnose-decode", "decode-parity", "bench-decode", "serve", "state-prefill", "state-restore", "state-roundtrip", "verify-rlvr"):
        ...
        p.add_argument("--checkpoint", type=Path, default=Path("neuromamba/runs/mamba3_tiny/model.pt"))
        p.add_argument("--state-out", type=Path, default=Path("neuromamba/runs/mamba3_tiny/state.pt"))

    sub.choices["train-tiny"].add_argument("--data", nargs="+", type=Path, default=[Path("luma/data/train_dialogues.txt")])
    ...
    sub.choices["train-packed"].add_argument("--data", nargs="+", type=Path, default=[Path("luma/data/english_bootstrap.txt")])
    ...
    sub.choices["train-answer"].add_argument("--data", nargs="+", type=Path, default=[Path("neuromamba/data/mamba3_programmatic_curriculum.jsonl")])
    ...
    sub.choices["train-multitask"].add_argument("--base-data", nargs="+", type=Path, default=[Path("luma/data/english_bootstrap.txt")])
    sub.choices["train-multitask"].add_argument("--answer-data", nargs="+", type=Path, default=[Path("neuromamba/data/mamba3_programmatic_curriculum.jsonl")])
    ...
    sub.choices["probe-kernel"].add_argument("--data", nargs="+", type=Path, default=[Path("luma/data/english_bootstrap.txt")])
    sub.choices["eval-loss"].add_argument("--data", nargs="+", type=Path, default=[Path("luma/data/english_bootstrap.txt")])
    sub.choices["eval-loss"].add_argument("--batches", type=int, default=8)
    sub.choices["eval-answer-loss"].add_argument("--data", nargs="+", type=Path, default=[Path("neuromamba/data/mamba3_programmatic_curriculum.jsonl")])
    ...
    sub.choices["state-restore"].add_argument("--state-in", type=Path, default=Path("neuromamba/runs/mamba3_tiny/state.pt"))
    sub.choices["state-roundtrip"].add_argument("--text", default="Project state checkpoint test for Mamba-3 recurrent memory.")
    sub.choices["state-roundtrip"].add_argument("--state-in", type=Path, default=Path("neuromamba/runs/mamba3_tiny/state_roundtrip.pt"))
    sub.choices["verify-rlvr"].add_argument("--rlvr-data", type=Path, default=Path("luma/data/rlvr_verifier_bootstrap.jsonl"))

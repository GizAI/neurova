...
case "${1:-status}" in
  start)
    ...
    setsid neuromamba/scripts/mamba3_current_apply_watcher.sh > "${log}" 2>&1 < /dev/null &
    pid=$!
    printf '%s\n' "${pid}" > "${PID_FILE}"
  ...
  status)
    ...
    echo "checkpoint_mtime=$(stat -c '%Y' neuromamba/runs/mamba3_current/model.pt 2>/dev/null || echo missing)"

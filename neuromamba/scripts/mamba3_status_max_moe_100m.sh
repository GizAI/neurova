...
echo "== process =="
ps -eo pid,etime,cmd | grep -E "mamba3_train_clean_doc_until_gate|neuromamba.cli train-packed --mode mimo-r4-moe-2.4b" | grep -v grep || true
train_pid="$(ps -eo pid,cmd | awk '/neuromamba.cli train-packed --mode mimo-r4-moe-2.4b/ && !/awk/ {print $1; exit}')"

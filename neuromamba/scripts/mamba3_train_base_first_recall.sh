...
DEVICE="${DEVICE:-cuda}"
DTYPE="${DTYPE:-bf16}"
RUN_DIR="${RUN_DIR:-neuromamba/runs/mamba3_recall_r2_base_first}"
LOG_FILE="${LOG_FILE:-${RUN_DIR}/train.log}"
PROGRAMMATIC_DATA="${PROGRAMMATIC_DATA:-neuromamba/data/mamba3_programmatic_curriculum_base_first.jsonl}"
...
cd "${ROOT}"
mkdir -p neuromamba/data/splits "${RUN_DIR}"
...
payload = {
    ...
    "programmatic_data": os.environ.get("PROGRAMMATIC_DATA", "neuromamba/data/mamba3_programmatic_curriculum_base_first.jsonl"),
...
if [[ ! -f neuromamba/data/clean_english_sft_bootstrap_v1.jsonl ]]; then
  python neuromamba/scripts/mamba3_generate_clean_english_sft.py \
    --out neuromamba/data/clean_english_sft_bootstrap_v1.jsonl \
    --records 1600 \
    --seed 20260613
fi
python neuromamba/scripts/mamba3_make_splits.py \
  --inputs neuromamba/data/governed_fineweb_edu_sample.jsonl neuromamba/data/governed_dclm_sample.jsonl neuromamba/data/clean_english_sft_bootstrap_v1.jsonl \
  --train-out neuromamba/data/splits/base_first_train.txt \
  --valid-out neuromamba/data/splits/base_first_valid.txt \
  --valid-ratio 0.02 \
  --seed 1337
...
python neuromamba/scripts/mamba3_generate_programmatic_curriculum.py "${GEN_CURRICULUM_ARGS[@]}"
python neuromamba/scripts/mamba3_make_splits.py \
  --inputs "${PROGRAMMATIC_DATA}" \
  --train-out neuromamba/data/splits/base_first_curriculum_train.txt \
  --valid-out neuromamba/data/splits/base_first_curriculum_valid.txt \
  --valid-ratio 0.10 \
...
python -m neuromamba.cli train-packed \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/base_first_train.txt \
...
python -m neuromamba.cli eval-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/base_first_valid.txt \
  --checkpoint "${BASE_CKPT}" \
  --batches 16
...
if [[ "${CURRICULUM_STEPS}" != "0" ]]; then
  ...
  if [[ "${CURRICULUM_LOSS}" == "answer" ]]; then
    python -m neuromamba.cli train-answer \
      ...
      --device "${DEVICE}" \
      --dtype "${DTYPE}" \
      --data neuromamba/data/splits/base_first_curriculum_train.txt \
    ...
    python -m neuromamba.cli train-multitask \
      ...
      --device "${DEVICE}" \
      --dtype "${DTYPE}" \
      --base-data neuromamba/data/splits/base_first_train.txt \
      --answer-data neuromamba/data/splits/base_first_curriculum_train.txt \
      --checkpoint "${CURRICULUM_CKPT}" \
      --save-every 100 \
      --no-resume
  ...
  if [[ "${CURRICULUM_LOSS}" == "answer" || "${CURRICULUM_LOSS}" == "multitask" ]]; then
    python -m neuromamba.cli eval-answer-loss \
      ...
      --device "${DEVICE}" \
      --dtype "${DTYPE}" \
      --data neuromamba/data/splits/base_first_curriculum_valid.txt \
      --checkpoint "${CURRICULUM_CKPT}" \
      --batches 16

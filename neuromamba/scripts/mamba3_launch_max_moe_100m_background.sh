...
if pgrep -f "neuromamba.cli train-packed --mode mimo-r4-moe-2.4b" >/dev/null; then
  echo "A mimo-r4-moe-2.4b train-packed process is already running:" >&2
  ps -eo pid,etime,cmd | grep "neuromamba.cli train-packed --mode mimo-r4-moe-2.4b" | grep -v grep >&2
  exit 2
fi
...
train_pid="$(pgrep -f "neuromamba.cli train-packed --mode mimo-r4-moe-2.4b" | tail -n 1 || true)"
if [[ -z "${train_pid}" ]]; then

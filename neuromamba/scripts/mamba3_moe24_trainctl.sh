...
RUN_DIR="${RUN_DIR:-${ROOT}/neuromamba/runs/mamba3_clean_doc_base_moe24_v1}"
CONTROL_DIR="${CONTROL_DIR:-${RUN_DIR}/control}"
LONG_DIR="${LONG_DIR:-${RUN_DIR}/long_blocks}"
MODE="${MODE:-mimo-r4-moe-2.4b}"
TRAIN_PATTERN="neuromamba.cli train-packed --mode ${MODE}"
...
usage() {
  cat <<'EOF'
Usage: neuromamba/scripts/mamba3_moe24_trainctl.sh <command>
...
status() {
  ...
  if [[ -n "${log}" ]]; then
    ...
    LOG="${log}" "${ROOT}/neuromamba/scripts/mamba3_status_max_moe_100m.sh"
  else
    "${ROOT}/neuromamba/scripts/mamba3_status_max_moe_100m.sh"
  fi
}
...
exec neuromamba/scripts/mamba3_train_max_moe_100m_block.sh
EOF
...
decision() {
  python "${ROOT}/neuromamba/scripts/mamba3_post100m_decision.py" \
...
router_diagnostics() {
  ...
  python "${ROOT}/neuromamba/scripts/mamba3_moe_router_diagnostics.py" \
    --mode "${MODE}" \
    --tokenizer "${TOKENIZER:-llama31}" \
    --checkpoint "${CHECKPOINT:-${RUN_DIR}/base.pt}" \
    --data "${VALID_DATA:-neuromamba/data/splits/base_doc_cont_v3_valid.jsonl}" \
...
watchdog_start() {
  ...
  setsid "${ROOT}/neuromamba/scripts/mamba3_moe24_trainctl.sh" watchdog-loop > "${log}" 2>&1 < /dev/null &
  pid=$!

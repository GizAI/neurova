...
promote_if_better() {
  ...
  local current_meta="${ROOT}/neuromamba/runs/mamba3_current/autoloop_metadata.json"
...
if not ok or rate < best:
    sys.exit(1)
PY
  mkdir -p "${ROOT}/neuromamba/runs/mamba3_current"
  cp "${model}" "${ROOT}/neuromamba/runs/mamba3_current/model.pt"
  cp "${gate}" "${ROOT}/neuromamba/runs/mamba3_current/chat_quality_gate.json"
  [[ -f "${tune}" ]] && cp "${tune}" "${ROOT}/neuromamba/runs/mamba3_current/decode_tune.json"
...
payload = {
    ...
    "promoted": "neuromamba/runs/mamba3_current/model.pt",
}
...
trial_params() {
  local trial="$1"
  case "${trial}" in
    1) echo "STEPS=4500 LR=1.5e-5 RECORDS=60000 START_CHECKPOINT=neuromamba/runs/mamba3_neurova_chat_v1/chat.pt" ;;
    2) echo "STEPS=6000 LR=1.0e-5 RECORDS=90000 START_CHECKPOINT=neuromamba/runs/mamba3_current/model.pt" ;;
    3) echo "STEPS=4500 LR=8e-6 RECORDS=120000 START_CHECKPOINT=neuromamba/runs/mamba3_current/model.pt" ;;
    4) echo "STEPS=3000 LR=6e-6 RECORDS=120000 START_CHECKPOINT=neuromamba/runs/mamba3_current/model.pt" ;;
    *) echo "STEPS=3000 LR=5e-6 RECORDS=120000 START_CHECKPOINT=neuromamba/runs/mamba3_current/model.pt" ;;
  esac
}
...
wait_for_existing
if [[ -f neuromamba/runs/mamba3_neurova_chat_v1/chat.pt ]]; then
  promote_if_better "neuromamba/runs/mamba3_neurova_chat_v1" || true
fi
...
for trial in $(seq 1 "${MAX_TRIALS}"); do
  ...
  echo "trial=${trial} params=${params} log=${log}"
  {
    ...
    if [[ ! -f "${START_CHECKPOINT}" ]]; then
      START_CHECKPOINT="neuromamba/runs/mamba3_neurova_speak_v1/sft.pt"
    fi
    RUN_DIR="${trial_dir}" \
    ...
    neuromamba/scripts/mamba3_train_neurova_chat_v1.sh
  } >"${log}" 2>&1 || true

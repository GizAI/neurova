...
FALLBACK_CHECKPOINT="${NEUROVA_MAMBA3_FALLBACK_CHECKPOINT:-neuromamba/runs/mamba3_neurova_chat_v1/chat.pt}"
SECOND_FALLBACK_CHECKPOINT="${NEUROVA_MAMBA3_SECOND_FALLBACK_CHECKPOINT:-neuromamba/runs/mamba3_neurova_speak_v1/sft.pt}"
...
RUN_DIR="${NEUROVA_MAMBA3_SERVER_RUN_DIR:-neuromamba/runs/mamba3_chat_server}"
...
case "$ACTION" in
  start)
    ...
    write_config
    nohup python neuromamba/scripts/mamba3_chat_server.py \
      --host "$HOST" \
      --port "$PORT" \
      --mode "$MODE" \
      --tokenizer "$TOKENIZER" \

...
RUN_ROOT="${RUN_ROOT:-neuromamba/runs/mamba3_teacher_research/deepseek_v4_pro_0_3b}"
...
PY
  ...
  if [[ "${round}" == "1" && "${BOOTSTRAP_EXISTING}" == "1" ]]; then
    ...
    python neuromamba/scripts/mamba3_generate_no_cheat_mcq_sft.py \
      ...
      --seed 20260614
    python neuromamba/scripts/mamba3_build_teacher_mcq_mix.py \
      --inputs \
        neuromamba/data/deepseek_no_cheat_mcq_sft_v1.jsonl \
  ...
  neuromamba/scripts/mamba3_train_self_teacher_mcq_v1.sh
  ...
  if better_than_best "${final_summary}"; then
    ...
    if [[ "${PROMOTE_CURRENT}" == "1" ]] && [[ "$(json_field "${final_gate}" ok false)" == "True" ]]; then
      mkdir -p neuromamba/runs/mamba3_current
      cp "${final_ckpt}" neuromamba/runs/mamba3_current/model.pt
      cp "${final_summary}" neuromamba/runs/mamba3_current/teacher_research_metadata.json
      [[ -f "${final_gate}" ]] && cp "${final_gate}" neuromamba/runs/mamba3_current/chat_quality_gate.json
      [[ -f "${final_mmlu}" ]] && cp "${final_mmlu}" neuromamba/runs/mamba3_current/mmlu_redux_sample.json
      if [[ "${AUTO_RESTART_SERVER}" == "1" ]]; then
        NEUROVA_MAMBA3_CHECKPOINT=neuromamba/runs/mamba3_current/model.pt \

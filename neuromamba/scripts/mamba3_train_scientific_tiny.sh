...
DEVICE="${DEVICE:-cuda}"
DTYPE="${DTYPE:-bf16}"
RUN_DIR="${RUN_DIR:-neuromamba/runs/mamba3_scientific}"
LOG_FILE="${LOG_FILE:-${RUN_DIR}/train.log}"
PROGRAMMATIC_DATA="${PROGRAMMATIC_DATA:-neuromamba/data/mamba3_programmatic_curriculum.jsonl}"
...
payload = {
    ...
    "programmatic_data": os.environ.get("PROGRAMMATIC_DATA", "neuromamba/data/mamba3_programmatic_curriculum.jsonl"),
...
python neuromamba/scripts/mamba3_make_splits.py \
  ...
  --train-out neuromamba/data/splits/base_train.jsonl \
  --valid-out neuromamba/data/splits/base_valid.jsonl \
  --valid-ratio 0.02 \
  --seed 1337
...
python neuromamba/scripts/mamba3_build_sft_mix.py \
  --natural-inputs luma/data/english_instruction_bootstrap.txt luma/data/english_completion_bootstrap.txt \
  --programmatic-inputs "${PROGRAMMATIC_DATA}" \
  --train-out neuromamba/data/splits/sft_train.txt \
  --valid-out neuromamba/data/splits/sft_valid.txt \
...
if [[ "${CURRICULUM_STEPS}" != "0" ]]; then
  ...
  python neuromamba/scripts/mamba3_make_splits.py \
    ...
    --train-out neuromamba/data/splits/curriculum_train.txt \
    --valid-out neuromamba/data/splits/curriculum_valid.txt \
    --valid-ratio 0.10 \
...
python -m neuromamba.cli train-packed \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/base_train.jsonl \
...
python -m neuromamba.cli eval-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/base_valid.jsonl \
  --checkpoint "${BASE_CKPT}" \
  --batches 8
...
if [[ "${CURRICULUM_STEPS}" != "0" ]]; then
  ...
  python -m neuromamba.cli train-packed \
    ...
    --device "${DEVICE}" \
    --dtype "${DTYPE}" \
    --data neuromamba/data/splits/curriculum_train.txt \
  ...
  python -m neuromamba.cli eval-loss \
    ...
    --device "${DEVICE}" \
    --dtype "${DTYPE}" \
    --data neuromamba/data/splits/curriculum_valid.txt \
    --checkpoint "${CURRICULUM_CKPT}" \
    --batches 8
...
python -m neuromamba.cli train-packed \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/sft_train.txt \
  --checkpoint "${SFT_CKPT}" \
  --save-every 50 \
  --no-resume \
  "${DS_FLAG[@]}" \
...
python -m neuromamba.cli eval-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/sft_valid.txt \
  --checkpoint "${SFT_CKPT}" \
  --batches 8

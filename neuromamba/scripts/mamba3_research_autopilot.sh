...
MOE_RUN_DIR="${MOE_RUN_DIR:-${ROOT}/neuromamba/runs/mamba3_clean_doc_base_moe24_v1}"
MOE_CTL="${MOE_CTL:-${ROOT}/neuromamba/scripts/mamba3_moe24_trainctl.sh}"
...
training_pid() {
  ps -eo pid,cmd | awk '/neuromamba.cli train-packed --mode mimo-r4-moe-2.4b/ && !/awk/ {print $1; exit}'
}
...
stop_servers_for_training() {
  ...
  for port in ${MAMBA3_SERVER_PORTS}; do
    NEUROVA_MAMBA3_SERVER_PORT="${port}" neuromamba/scripts/mamba3_chat_serverctl.sh stop >/dev/null 2>&1 || true
  done
  local pid
...
write_snapshot() {
  local snapshot="${CONTROL_DIR}/latest_status.txt"
  {
    ...
    echo
    echo "== processes =="
    ps -eo pid,etime,cmd | grep -E "mamba3_chat_server|mamba3_moe24_trainctl.sh watchdog-loop|neuromamba.cli train-packed --mode mimo-r4-moe-2.4b" | grep -v grep || true
    echo
    echo "== gpu =="

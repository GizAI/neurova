...
RUN_DIR="${RUN_DIR:-neuromamba/runs/mamba3_siso_fast_0_3b_v1}"
...
python neuromamba/scripts/mamba3_generate_neurova_chat_sft.py \
  --out neuromamba/data/neurova_siso_hybrid_sft_v1.jsonl \
  --records "${RECORDS}" \
  --seed 20260613

python neuromamba/scripts/mamba3_generate_state_memory_curriculum.py \
  --out neuromamba/data/neurova_state_memory_curriculum_v1.jsonl \
...
python neuromamba/scripts/mamba3_make_splits.py \
  --inputs neuromamba/data/neurova_siso_hybrid_sft_v1.jsonl neuromamba/data/neurova_state_memory_curriculum_v1.jsonl \
  --train-out neuromamba/data/splits/neurova_siso_hybrid_v1_train.jsonl \
  --valid-out neuromamba/data/splits/neurova_siso_hybrid_v1_valid.jsonl \
  --valid-ratio 0.05 \
  --seed 20260613
...
python -m neuromamba.cli train-answer \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/neurova_siso_hybrid_v1_train.jsonl \
...
python -m neuromamba.cli eval-answer-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/neurova_siso_hybrid_v1_valid.jsonl \
  --checkpoint "${CHECKPOINT}" \

...
RUN_DIR="${RUN_DIR:-neuromamba/runs/mamba3_neurova_chat_v1}"
START_CHECKPOINT="${START_CHECKPOINT:-neuromamba/runs/mamba3_neurova_speak_v1/sft.pt}"
FALLBACK_CHECKPOINT="${FALLBACK_CHECKPOINT:-neuromamba/runs/mamba3_tiny/model_mimo_r4_speak.pt}"
CHECKPOINT="${CHECKPOINT:-${RUN_DIR}/chat.pt}"
LOG_FILE="${LOG_FILE:-${RUN_DIR}/train.log}"
CONTINUE_EXISTING="${CONTINUE_EXISTING:-0}"

cd "${ROOT}"
mkdir -p "${RUN_DIR}/checkpoints" neuromamba/data/splits
...
python neuromamba/scripts/mamba3_generate_neurova_chat_sft.py \
  --out neuromamba/data/neurova_chat_sft_v1.jsonl \
  --records "${RECORDS}" \
  --seed 20260613

python neuromamba/scripts/mamba3_make_splits.py \
  --inputs neuromamba/data/neurova_chat_sft_v1.jsonl \
  --train-out neuromamba/data/splits/neurova_chat_v1_train.jsonl \
  --valid-out neuromamba/data/splits/neurova_chat_v1_valid.jsonl \
  --valid-ratio 0.05 \
  --seed 20260613
...
python -m neuromamba.cli train-answer \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/neurova_chat_v1_train.jsonl \
...
python -m neuromamba.cli eval-answer-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data neuromamba/data/splits/neurova_chat_v1_valid.jsonl \
  --checkpoint "${CHECKPOINT}" \
  --batches 64 | tee "${RUN_DIR}/eval_answer_loss.json"

echo "== chat quality gate =="
timeout "${CHAT_GATE_TIMEOUT_SECONDS:-900}" python neuromamba/scripts/mamba3_chat_quality_gate.py \
  --mode "${MODE}" \
  --tokenizer "${TOKENIZER}" \
  --checkpoint "${CHECKPOINT}" \
  --seq-len "${SEQ_LEN}" \
...
echo "== decode tune =="
timeout "${DECODE_TUNE_TIMEOUT_SECONDS:-900}" python neuromamba/scripts/mamba3_decode_tune.py \
  --mode "${MODE}" \
  --tokenizer "${TOKENIZER}" \
  --checkpoint "${CHECKPOINT}" \
  --seq-len "${SEQ_LEN}" \

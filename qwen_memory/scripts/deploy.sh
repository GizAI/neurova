#!/usr/bin/env bash
set -euo pipefail
# Qwen Memory — Deploy & Run on ml-dmc8
# Usage: bash qwen_memory/scripts/deploy.sh

HOST="ml-dmc8"
NEUROVA_DIR="/home/user/workspace/neurova"
CONDA_ENV="qwen_memory"
SCRIPT="qwen_memory/main.py"

echo "=== Qwen Memory Deploy ==="

# 1. Sync code
echo "[1/4] Syncing code..."
rsync -avz --no-owner --no-group \
  "$NEUROVA_DIR/qwen_memory/" \
  "$HOST:$NEUROVA_DIR/qwen_memory/" 2>/dev/null || true

# Also scp as fallback
scp -r "$NEUROVA_DIR/qwen_memory" "$HOST:$NEUROVA_DIR/" 2>/dev/null || {

...
DEVICE="${DEVICE:-cuda}"
DTYPE="${DTYPE:-bf16}"
RUN_DIR="${RUN_DIR:-runs/mamba3_siso_fast_0_3b_ds128_intel_v1}"
CHECKPOINT="${CHECKPOINT:-${RUN_DIR}/model.pt}"
START_CHECKPOINT="${START_CHECKPOINT:-runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt}"
LOG_FILE="${LOG_FILE:-${RUN_DIR}/train.log}"
BASE_DATA="${BASE_DATA:-data/splits/base_doc_cont_v3_train.jsonl}"
FALLBACK_BASE_DATA="${FALLBACK_BASE_DATA:-data/english_completion_bootstrap.txt}"
...
python scripts/mamba3_generate_neurova_chat_sft.py \
  --out data/neurova_siso_fast_intel_chat_v1.jsonl \
  --records "${CHAT_RECORDS}" \
  --seed 20260613

python scripts/mamba3_generate_state_memory_curriculum.py \
  --out data/neurova_siso_fast_intel_state_v1.jsonl \
  --records "${STATE_MEMORY_RECORDS}" \
  --seed 20260614

python scripts/mamba3_make_splits.py \
  --inputs data/neurova_siso_fast_intel_chat_v1.jsonl data/neurova_siso_fast_intel_state_v1.jsonl \
  --train-out data/splits/neurova_siso_fast_intel_v1_train.jsonl \
  --valid-out data/splits/neurova_siso_fast_intel_v1_valid.jsonl \
  --valid-ratio 0.05 \
  --seed 20260613
...
python -m mamba3_kr.cli train-multitask \
  ...
  --dtype "${DTYPE}" \
  --base-data "${BASE_DATA}" \
  --answer-data data/splits/neurova_siso_fast_intel_v1_train.jsonl \
...
python -m mamba3_kr.cli eval-answer-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data data/splits/neurova_siso_fast_intel_v1_valid.jsonl \
  --checkpoint "${CHECKPOINT}" \

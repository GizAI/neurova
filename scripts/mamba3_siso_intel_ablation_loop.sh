...
RUN_ROOT="${RUN_ROOT:-runs/mamba3_siso_intel_ablation}"
...
BASE_DATA="${BASE_DATA:-data/splits/no_cheat_knowledge_v1_train.jsonl}"
ANSWER_DATA="${ANSWER_DATA:-data/splits/no_cheat_mcq_sft_v1_train.jsonl}"
STEPS="${STEPS:-300}"
SEQ_LEN="${SEQ_LEN:-256}"
...
INIT_CHECKPOINT="${INIT_CHECKPOINT:-runs/mamba3_current/model.pt}"

mkdir -p "${RUN_ROOT}"

if [[ "${WAIT_FOR_TEACHER}" == "1" ]]; then
  while pgrep -f "scripts/mamba3_teacher_research_loop.sh|mamba3_kr.cli train-|mamba3_eval_mcq_bench.py|mamba3_chat_quality_gate.py|mamba3_train_neurova_chat_v1.sh" >/dev/null; do
...
for mode in "${MODES[@]}"; do
  ...
  if [[ -f "${INIT_CHECKPOINT}" ]]; then
    python scripts/mamba3_transplant_checkpoint.py \
  ...
  python -m mamba3_kr.cli eval-answer-loss \
    ...
    --tokenizer "${TOKENIZER}" \
    --checkpoint "${ckpt}" \
    --data data/splits/no_cheat_mcq_sft_v1_valid.jsonl \
    --seq-len "${SEQ_LEN}" \
    --batch-size "${BATCH_SIZE}" \
    --device "${DEVICE}" \
    --dtype "${DTYPE}" \

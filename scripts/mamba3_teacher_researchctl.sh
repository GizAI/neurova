...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/mamba3_teacher_research/deepseek_v4_pro_0_3b}"
...
active_legacy_pid() {
  ps -eo pid,cmd | awk '
    ...
    index($0, "data/deepseek_no_cheat_mcq_sft_v1.jsonl") &&
...
exec scripts/mamba3_teacher_research_loop.sh
EOF
    scripts/mamba3_current_applyctl.sh start >/dev/null 2>&1 || true
  ...
  status)
    ...
    echo "== data progress =="
    find "${RUN_ROOT}" -name 'deepseek_no_cheat_mcq.jsonl' -o -path "${ROOT}/data/deepseek_no_cheat_mcq_sft_v1.jsonl" 2>/dev/null \

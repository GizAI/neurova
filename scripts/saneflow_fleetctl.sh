...
case "$cmd" in
  status)
    ...
    echo
    echo "== dmc9: active lines B/C, practical-base + R champion =="
  ...
  active)
    echo "== dmc8 active =="
    remote "$DMC8" "python scripts/saneflow_run.py status dmc8-speak-base-v1 dmc8-chatml-sft-v9; echo; nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu,power.draw --format=csv,noheader || true"
    echo
    echo "== dmc9 active =="
    remote "$DMC9" "/home/user/miniconda3/envs/saneflow/bin/python scripts/saneflow_run.py status dmc9-practical-base-100m dmc9-r-champion-delta-landmark-long dmc9-r-champion-practical-cont; echo; nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu,power.draw --format=csv,noheader || true"
    ;;
  start)
  ...
  logs)
    ...
    ;;
  audit)
    remote "$DMC8" "python scripts/saneflow_data_audit.py --out runs/saneflow_data_audit/dmc8_audit.json --sample-rows 5000"
    remote "$DMC9" "/home/user/miniconda3/envs/saneflow/bin/python scripts/saneflow_data_audit.py --out runs/saneflow_data_audit/dmc9_audit.json --sample-rows 5000"
    ;;
  *)

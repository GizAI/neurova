...
scripts/mamba3_infer_guard.sh run scripts/mamba3_chat_serverctl.sh start >/dev/null
...
while true; do
  ...
  printf "neurova> "
  scripts/mamba3_infer_guard.sh run python scripts/mamba3_chat_client.py \

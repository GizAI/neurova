...
def write_local_fallback(out: Path, max_records: int, min_chars: int, max_chars: int, needles: set[str]) -> int:
    paths = [
        Path("data/splits/base_doc_cont_v3_train.jsonl"),
        Path("data/english_completion_bootstrap.txt"),
        Path("data/english_instruction_bootstrap.txt"),
    ]
    seen: set[str] = set()
    count = 0
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("data/no_cheat_knowledge_v1.jsonl"))

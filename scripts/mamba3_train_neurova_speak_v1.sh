...
DEVICE="${DEVICE:-cuda}"
DTYPE="${DTYPE:-bf16}"
RUN_DIR="${RUN_DIR:-runs/mamba3_neurova_speak_v1}"
START_CHECKPOINT="${START_CHECKPOINT:-runs/mamba3_kr_tiny/model_mimo_r4_speak.pt}"
...
payload = {
    ...
    "start_checkpoint": os.environ.get("START_CHECKPOINT", "runs/mamba3_kr_tiny/model_mimo_r4_speak.pt"),
    "checkpoint": os.environ.get("CHECKPOINT", "runs/mamba3_neurova_speak_v1/sft.pt"),
...
python scripts/mamba3_generate_neurova_speak_sft.py \
  --out data/neurova_speak_sft_v1.jsonl \
  --repeats "${SPEAK_REPEATS:-100}" \
  --seed 20260613

python scripts/mamba3_make_splits.py \
  --inputs data/neurova_speak_sft_v1.jsonl data/basic_english_qa_v1.jsonl data/clean_english_sft_bootstrap_v1.jsonl \
  --train-out data/splits/neurova_speak_v1_train.txt \
  --valid-out data/splits/neurova_speak_v1_valid.txt \
  --valid-ratio 0.08 \
  --seed 20260613
...
python -m mamba3_kr.cli train-answer \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data data/splits/neurova_speak_v1_train.txt \
...
python -m mamba3_kr.cli eval-answer-loss \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --data data/splits/neurova_speak_v1_valid.txt \
  --checkpoint "${CHECKPOINT}" \
...
do
  echo "--- prompt=${prompt}"
  python -m mamba3_kr.cli fast-generate \
    --mode "${MODE}" \
    --tokenizer "${TOKENIZER}" \
    --checkpoint "${CHECKPOINT}" \
    --prompt "${prompt}" \

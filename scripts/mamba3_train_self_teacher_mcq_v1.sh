...
RUN_DIR="${RUN_DIR:-runs/mamba3_self_teacher_mcq_v1}"
...
MCQ_DATA="${MCQ_DATA:-data/no_cheat_mcq_sft_v1.jsonl}"
DEEPSEEK_MCQ_DATA="${DEEPSEEK_MCQ_DATA:-data/deepseek_no_cheat_mcq_sft_v1.jsonl}"
...
if [[ "${TEACHER_PROVIDER}" == "deepseek" ]]; then
  ...
  export MCQ_DATA
  python scripts/mamba3_generate_deepseek_mcq_sft.py \
    --out "${MCQ_DATA}" \
...
python scripts/mamba3_make_splits.py \
  --inputs "${MCQ_DATA}" \
  --train-out data/splits/no_cheat_mcq_sft_v1_train.jsonl \
  --valid-out data/splits/no_cheat_mcq_sft_v1_valid.jsonl \
  --valid-ratio 0.03 \
  --seed 20260614
...
python -m mamba3_kr.cli train-multitask \
  ...
  --base-data "${BASE_DATA}" \
  --answer-data data/splits/no_cheat_mcq_sft_v1_train.jsonl \
  --steps "${STEPS}" \
  --lr "${LR}" \

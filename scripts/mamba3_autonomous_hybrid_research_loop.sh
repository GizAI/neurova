...
BASE_CHECKPOINT="${BASE_CHECKPOINT:-runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt}"
TRAIN_DATA="${TRAIN_DATA:-data/splits/no_cheat_knowledge_v1_train.jsonl}"
VALID_DATA="${VALID_DATA:-data/splits/no_cheat_knowledge_v1_valid.jsonl}"
PROGRAMMATIC_DATA="${PROGRAMMATIC_DATA:-data/mamba3_programmatic_curriculum_eval.jsonl}"
RUN_ROOT="${RUN_ROOT:-runs/mamba3_autonomous_hybrid_research/$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ ! -f "${TRAIN_DATA}" || ! -f "${VALID_DATA}" ]]; then
  echo "missing no-cheat splits; building from data/no_cheat_knowledge_v1.jsonl" >&2
  python scripts/mamba3_make_splits.py \
    --inputs data/no_cheat_knowledge_v1.jsonl \

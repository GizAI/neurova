...
echo "== runs =="
for run in \
  runs/luma_stage1_qwen_natural_pre_v1 \
  runs/luma_stage2_qwen_chat_sft_v1 \
  runs/luma_stage3_qwen_slotproof_v1
do
  echo "-- ${run}"
...
echo
echo "== pipeline =="
tail -n 10 runs/luma_research_pipeline_dmc8.log 2>/dev/null || true

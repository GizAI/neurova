...
RUN_DIR="${RUN_DIR:-runs/mamba3_siso_fast_0_3b_ds128_no_cheat_knowledge_v1}"
CHECKPOINT="${CHECKPOINT:-${RUN_DIR}/model.pt}"
START_CHECKPOINT="${START_CHECKPOINT:-runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt}"
CORPUS="${CORPUS:-data/no_cheat_knowledge_v1.jsonl}"
...
if [[ ! -f "${CORPUS}" ]]; then
  python scripts/mamba3_build_no_cheat_knowledge_corpus.py \
    --out "${CORPUS}" \
...
python scripts/mamba3_make_splits.py \
  --inputs "${CORPUS}" \
  --train-out data/splits/no_cheat_knowledge_v1_train.jsonl \
  --valid-out data/splits/no_cheat_knowledge_v1_valid.jsonl \
  --valid-ratio 0.01 \
  --seed 20260613
...
python -m mamba3_kr.cli train-packed \
  ...
  --data data/splits/no_cheat_knowledge_v1_train.jsonl \
  --steps "${STEPS}" \
  --lr "${LR}" \
...
python -m mamba3_kr.cli eval-loss \
  ...
  --tokenizer "${TOKENIZER}" \
  --checkpoint "${CHECKPOINT}" \
  --data data/splits/no_cheat_knowledge_v1_valid.jsonl \
  --seq-len "${SEQ_LEN}" \
  --batch-size "${BATCH_SIZE}" \
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \

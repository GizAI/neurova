...
def iter_candidates(root: Path, out_dir: Path) -> list[Path]:
    exts = {".jsonl", ".json", ".txt"}
    out: list[Path] = []
    for base in [root / "data", root / "configs"]:
        if not base.exists():
            continue
...
def main() -> None:
    ...
    for src in iter_candidates(root, out_dir):
        ...
        if src.is_relative_to(out_dir):
            dst_rel = rel.relative_to(Path("data/corpus"))
            dst = src
            link_type = "in_place"
        else:
        ...
        if duplicate_of is None:
            ...
            by_hash[digest] = {
                "canonical_relpath": str(Path("data/corpus") / dst_rel),
                "source_relpath": str(rel),
            }
        entries.append(
            {
                "source_relpath": str(rel),
                "canonical_relpath": None if duplicate_of else str(Path("data/corpus") / dst_rel),
    ...
    registry = {
        ...
        "policy": {
            "single_source_of_truth": "data/corpus/registry.json",

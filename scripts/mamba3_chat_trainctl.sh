...
RUN_DIR="${RUN_DIR:-${ROOT}/runs/mamba3_neurova_chat_v1}"
...
usage() {
  cat <<'EOF'
Usage: scripts/mamba3_chat_trainctl.sh <command>
...
Commands:
  ...
  promote   Promote chat.pt to runs/mamba3_current/model.pt if gate passed.
...
export START_CHECKPOINT="${START_CHECKPOINT:-runs/mamba3_neurova_speak_v1/sft.pt}"
exec scripts/mamba3_train_neurova_chat_v1.sh
EOF
...
promote() {
  ...
  local dst="${ROOT}/runs/mamba3_current/model.pt"
...
PY
  ...
  [[ -f "${RUN_DIR}/decode_tune/latest.json" ]] && cp "${RUN_DIR}/decode_tune/latest.json" "${ROOT}/runs/mamba3_current/decode_tune.json"
  echo "promoted=${dst}"
}

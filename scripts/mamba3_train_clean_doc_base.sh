...
OVERFIT_DATA="${OVERFIT_DATA:-data/splits/base_doc_cont_overfit64.txt}"

RUN_DIR="${RUN_DIR:-runs/mamba3_clean_doc_base_moe900_v1}"
...
payload = {
    ...
    "doc_corpus": os.environ.get("DOC_CORPUS", "data/base_doc_continuation_v1.jsonl"),
    "train_data": os.environ.get("TRAIN_DATA", "data/splits/base_doc_cont_train.txt"),
    "valid_data": os.environ.get("VALID_DATA", "data/splits/base_doc_cont_valid.txt"),
    "overfit_data": os.environ.get("OVERFIT_DATA", "data/splits/base_doc_cont_overfit64.txt"),
...
if [[ ! -f "${DOC_CORPUS}" ]]; then
  echo "missing DOC_CORPUS=${DOC_CORPUS}; build it with scripts/mamba3_build_doc_continuation_corpus.py" >&2
  exit 2
fi
...
train = Path(os.environ.get("TRAIN_DATA", "data/splits/base_doc_cont_train.txt"))
out = Path(os.environ.get("OVERFIT_DATA", "data/splits/base_doc_cont_overfit64.txt"))

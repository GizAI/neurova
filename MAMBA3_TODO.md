...
- [x] Add top-level autonomous research supervisor.
  - Script: `scripts/mamba3_research_autopilot.sh`.
  - Controller: `scripts/mamba3_research_autopilotctl.sh`.
  - User entrypoints:
  ...
  - It keeps `scripts/mamba3_moe24_trainctl.sh watchdog-start` active and calls `resume` if the training process disappears.
  - It writes `runs/mamba3_research_autopilot/control/latest_status.txt` with process, GPU, training progress, and decision output.
...
- [x] Add a no-cheat multiple-choice benchmark harness.
  - Script: `scripts/mamba3_eval_mcq_bench.py`.
...
- [x] Fix benchmark GPU ownership.
  - Root cause: `scripts/mamba3_infer_guard.sh` used `SIGSTOP`, which pauses compute but keeps the 2.4B training process' CUDA memory allocated. MMLU/MMLU-Redux then OOMed during Triton Mamba kernel autotune.
  - Fix: `scripts/mamba3_exclusive_gpu_guard.sh` stops autopilot/watchdog/training, runs the benchmark with free VRAM, then restarts the previously active loop.
...
- [x] Add a combined benchmark suite.
  - Script: `scripts/mamba3_benchmark_suite.sh`.
  - User command: `./neurova.sh mamba3 bench-suite`.
  - Outputs under `runs/mamba3_benchmarks/<timestamp>/` and writes `runs/mamba3_benchmarks/latest_summary.json`.
- [x] Run the initial real intelligence baseline for the current v3 fast checkpoint.
  - Checkpoint: `runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt`.
...
- [x] Add deterministic higher-quality Neurova-chat SFT corpus generator.
  - Script: `scripts/mamba3_generate_neurova_chat_sft.py`.
...
- [x] Add Neurova-chat v1 training and management scripts.
  - Training script: `scripts/mamba3_train_neurova_chat_v1.sh`.
  - Controller: `scripts/mamba3_chat_trainctl.sh`.
  - Default seed checkpoint: `runs/mamba3_neurova_speak_v1/sft.pt`, falling back to `runs/mamba3_kr_tiny/model_mimo_r4_speak.pt`.
  - Output checkpoint: `runs/mamba3_neurova_chat_v1/chat.pt`.
- [x] Add chat-specific quality gate.
  - Script: `scripts/mamba3_chat_quality_gate.py`.
...
- [x] Add 24-hour autonomous chat improvement loop.
  - Loop: `scripts/mamba3_chat_autoloop.sh`.
  - Controller: `scripts/mamba3_chat_autoloopctl.sh`.
  ...
  - Current user-facing default in `neurova.sh` resolves `runs/mamba3_current/model.pt` first, then chat v1, then speak v1.
- [x] Promote first usable Neurova-chat v1 checkpoint.
  - Source: `runs/mamba3_neurova_chat_v1/chat.pt`.
  - Gate: `pass_rate=0.9333`.
  - Promoted target: `runs/mamba3_current/model.pt`.
- [ ] Let the autonomous chat loop continue through its scheduled trials.
  - Monitor: `./neurova.sh mamba3 status` or `scripts/mamba3_chat_autoloopctl.sh status` on `ml-dmc8`.
...
- [x] Pause and preserve the 2.4B max-parameter MoE line instead of continuing blind training.
  - Remote command used: `scripts/mamba3_moe24_trainctl.sh stop` and `watchdog-stop`.
  - Preserved checkpoint: `runs/mamba3_clean_doc_base_moe24_v1/base.pt`.
...
- [x] Select the fastest viable speaking seed checkpoint.
  - Candidate selected: `runs/mamba3_kr_tiny/model_mimo_r4_speak.pt`.
...
- [x] Add deterministic no-teacher speaking SFT data generator.
  - Script: `scripts/mamba3_generate_neurova_speak_sft.py`.
...
- [x] Add reproducible speaking-model training script.
  - Script: `scripts/mamba3_train_neurova_speak_v1.sh`.
  - Default output: `runs/mamba3_neurova_speak_v1/sft.pt`.
...
- [x] Add managed background controller for same-day speaking training.
  - Script: `scripts/mamba3_speak_trainctl.sh`.
  - Commands: `start`, `status`, `tail`, `stop`, `logs`.
  - Logs: `runs/mamba3_neurova_speak_v1/logs/*.log`.
- [ ] Run `scripts/mamba3_train_neurova_speak_v1.sh` on `ml-dmc8`.
...
- [ ] Promote to `runs/mamba3_current/model.pt` only if it answers without repetition collapse.
- [ ] After today’s speaking checkpoint is usable, return to the canonical long-term intelligence recipe.
  - Source of truth: `configs/mamba3_english_top_intelligence_recipe.json`.
...
- [x] Add promotion script for `runs/mamba3_current/model.pt`.
- [x] Add 16GB candidate presets:
  ...
  - `mimo-r4-moe-2.5b`
  - `mimo-r4-moe-2.9b`
- [x] Run `scripts/mamba3_find_16gb_train_target.sh` on `ml-dmc8`.
...
- [x] Add maximum-size base training entrypoint.
  - Script: `scripts/mamba3_train_max_moe_base.sh`.
  - Default: `MODE=mimo-r4-moe-2.4b`, `BATCH_SIZE=1`, `SEQ_LEN=2048`, `LR=3e-5`, `--no-save-optimizer`.
  - Canary: `BASE_STEPS=20 SAVE_EVERY=0 EVAL_BATCHES=4 LR=3e-5 DATA_SEED=990001 scripts/mamba3_train_max_moe_base.sh`.
  - Result: checkpoint saved at `runs/mamba3_clean_doc_base_moe24_v1/base.pt`; held-out loss 11.6719 over 4 batches; decode is random because this is only a from-scratch canary, not a trained model.
- [x] Add maximum-size weight-only continuation loop.
  - Script: `scripts/mamba3_continue_max_moe_base.sh`.
  - It is a thin wrapper around `scripts/mamba3_continue_clean_doc_base.sh` to avoid duplicating train/eval/decode logging logic.
  - Default: `MODE=mimo-r4-moe-2.4b`, `RUN_DIR=runs/mamba3_clean_doc_base_moe24_v1`, `BATCH_SIZE=1`, `LR=2e-5`, `STEPS=250`, `EVAL_BATCHES=16`, `NO_SAVE_OPTIMIZER=1`.
  - Verification run: `DATA_SEED=991001 STEPS=250 LR=2e-5 EVAL_BATCHES=16 scripts/mamba3_continue_max_moe_base.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe24_v1/continuations/20260613T010628Z_*`.
- [x] Add maximum-size until-gate loop and repetition-collapse guard.
  - Script: `scripts/mamba3_train_max_moe_until_gate.sh`.
  - It wraps the shared `scripts/mamba3_train_clean_doc_until_gate.sh` with 2.4B defaults.
  ...
  - Verification run: `MAX_ROUNDS=1 STEPS_PER_ROUND=500 LR=1.5e-5 EVAL_BATCHES=16 DATA_SEED_BASE=993000 scripts/mamba3_train_max_moe_until_gate.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe24_v1/continuations/20260613T011044Z_*` and `runs/mamba3_clean_doc_base_moe24_v1/until_gate/summary.jsonl`.
- [x] Verify collapse-aware summary on the next 2.4B round.
  - Run: `MAX_ROUNDS=1 STEPS_PER_ROUND=500 LR=1.2e-5 EVAL_BATCHES=16 DATA_SEED_BASE=994000 scripts/mamba3_train_max_moe_until_gate.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe24_v1/continuations/20260613T011617Z_*`.
- [x] Tighten repetition gate to evaluate generated continuation only.
  ...
  - Fix: `scripts/mamba3_train_clean_doc_until_gate.sh` now strips the prompt before calculating repeated-word runs and distinct generated words.
...
- [x] Verify continuation-only collapse summary on 2.4B.
  - Run: `MAX_ROUNDS=1 STEPS_PER_ROUND=500 LR=1.0e-5 EVAL_BATCHES=16 DATA_SEED_BASE=995000 scripts/mamba3_train_max_moe_until_gate.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe24_v1/continuations/20260613T012058Z_*`.
- [x] Confirm 2.4B collapse is general, not a single-prompt artifact.
  - Run: `python scripts/mamba3_eval_raw_decode_set.py --mode mimo-r4-moe-2.4b --tokenizer llama31 --checkpoint runs/mamba3_clean_doc_base_moe24_v1/base.pt`.
  - Result: science/history/code/math raw document starts all collapsed.
  - Summary: `collapsed_prompts=4`, `passed=false`.
  - Artifact: `runs/mamba3_clean_doc_base_moe24_v1/raw_decode_set/20260613T013052Z.jsonl`.
- [x] Add first sufficient maximum-parameter base-training block.
  - Script: `scripts/mamba3_train_max_moe_100m_block.sh`.
  - Detached launcher: `scripts/mamba3_launch_max_moe_100m_background.sh`.
  - Status helper: `scripts/mamba3_status_max_moe_100m.sh`.
  ...
  - Managed train controller: `scripts/mamba3_moe24_trainctl.sh`.
  ...
  - Controller artifacts: `runs/mamba3_clean_doc_base_moe24_v1/control/current.log`, `current.log.path`, `train.pid`, `wrapper.pid`, `state.json`.
...
- [ ] Let the active 100M-token 2.4B base block finish on `ml-dmc8`.
  ...
  - Log: `runs/mamba3_clean_doc_base_moe24_v1/long_blocks/20260613T013135Z_100m.log`.
  - Command record: `runs/mamba3_clean_doc_base_moe24_v1/long_blocks/20260613T013135Z_100m.cmd`.
  ...
  - Fix for subsequent rounds: `mamba3_kr.data.iter_packed_token_batches` now adaptively splits long texts before tokenization, `train-packed` has `--max-text-chars` default `65536` and `--max-text-tokens` default `120000`, and `scripts/mamba3_continue_clean_doc_base.sh` passes `MAX_TEXT_CHARS`/`MAX_TEXT_TOKENS`.
  ...
  - The already-running long block has been adopted by `scripts/mamba3_moe24_trainctl.sh adopt` without interrupting training. Current adopted log: `runs/mamba3_clean_doc_base_moe24_v1/long_blocks/20260613T013135Z_100m.log`.
  - Watchdog is running under `scripts/mamba3_moe24_trainctl.sh watchdog-loop`; if wrapper/train processes disappear before the planned block completes, it computes remaining rounds from the log and restarts detached training from the latest weight checkpoint.
  ...
  - SOTA-oriented post-100M decision gate is not duplicated here. Source of truth: `configs/mamba3_english_top_intelligence_recipe.json` field `post_100m_local_decision_gate`; operator view: `scripts/mamba3_moe24_trainctl.sh decision`.
  - Do not start SFT or QA training while this item is open.
- [x] Add a non-interrupting post-100M decision report.
  - Script: `scripts/mamba3_post100m_decision.py`.
  - Operator command: `scripts/mamba3_moe24_trainctl.sh decision`.
  - It reads `runs/mamba3_clean_doc_base_moe24_v1/until_gate/summary.jsonl`, estimates 100M progress, and returns one of:
...
- [x] Add post-100M MoE/router diagnostic artifact generation.
  - Script: `scripts/mamba3_moe_router_diagnostics.py`.
  - Operator command: `scripts/mamba3_moe24_trainctl.sh router-diagnostics`.
  ...
  - It saves artifacts under `runs/mamba3_clean_doc_base_moe24_v1/router_diagnostics/`.
...
- [x] Train the selected candidate through clean raw-document base sanity gates.
  - Script: `scripts/mamba3_train_clean_doc_base.sh`.
  - Run: `runs/mamba3_clean_doc_base_moe900_v1`.
...
- [x] Add standard clean-doc continuation/eval loop.
  - Script: `scripts/mamba3_continue_clean_doc_base.sh`.
  ...
  - Verification run: `STEPS=250 SAVE_EVERY=250 LR=1.5e-5 scripts/mamba3_continue_clean_doc_base.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe900_v1/continuations/20260613T002911Z_*`.
...
- [x] Expand raw document-continuation corpus without overwriting v1 evidence.
  - Command: `MAX_DOCS=20000 MAX_BYTES=500000000 TECH_RECORDS=0 CLEAN_RECORDS=0 DOC_CORPUS_OUT=data/base_doc_continuation_v2.jsonl TRAIN_OUT=data/splits/base_doc_cont_v2_train.txt VALID_OUT=data/splits/base_doc_cont_v2_valid.txt scripts/mamba3_expand_governed_base.sh`.
  ...
  - Size: `data/base_doc_continuation_v2.jsonl` about 365MB, train split about 346MB, valid split about 7.5MB.
...
- [x] Verify 900M continuation on v2 corpus.
  ...
  - Run: `TRAIN_DATA=data/splits/base_doc_cont_v2_train.txt VALID_DATA=data/splits/base_doc_cont_v2_valid.txt STEPS=250 SAVE_EVERY=250 LR=1.5e-5 scripts/mamba3_continue_clean_doc_base.sh`.
  - Result: v2 held-out loss 6.3311, peak VRAM 12.98GB, warm training throughput about 24K-26K tok/s.
  - Artifacts: `runs/mamba3_clean_doc_base_moe900_v1/continuations/20260613T003427Z_*`.
...
- [x] Add gate-driven repeated clean-doc training loop.
  - Script: `scripts/mamba3_train_clean_doc_until_gate.sh`.
  - Gate: held-out doc loss must be below target and raw decode must produce at least the minimum new-token count.
  - Verification run: `MAX_ROUNDS=1 STEPS_PER_ROUND=500 LR=1.2e-5 TRAIN_DATA=data/splits/base_doc_cont_v2_train.txt VALID_DATA=data/splits/base_doc_cont_v2_valid.txt scripts/mamba3_train_clean_doc_until_gate.sh`.
  - Result: v2 held-out loss 6.3174, new_tokens 4, gate failed as expected.
  - Summary artifact: `runs/mamba3_clean_doc_base_moe900_v1/until_gate/summary.jsonl`.
- [x] Fix repeated weight-only restart data-order bias.
  ...
  - Fix: `mamba3_kr.data.iter_packed_token_batches` now supports seeded text shuffling.
  - CLI: `train-packed --shuffle-texts --data-seed N`.
  - `scripts/mamba3_continue_clean_doc_base.sh` now defaults to `SHUFFLE_TEXTS=1` and records `data_seed` in metadata.
  - `scripts/mamba3_train_clean_doc_until_gate.sh` uses a different deterministic seed per round.
  - Verification run: `TRAIN_DATA=data/splits/base_doc_cont_v2_train.txt VALID_DATA=data/splits/base_doc_cont_v2_valid.txt STEPS=250 SAVE_EVERY=250 LR=1e-5 DATA_SEED=777001 scripts/mamba3_continue_clean_doc_base.sh`.
...
- [x] Fix document-boundary corruption in split files.
  ...
  - Evidence: `data/splits/base_doc_cont_v2_train.txt` had 77,673 `<doc>` records but 2,561,832 physical lines; the first sample read by the trainer was only `<doc ...>\n` followed by separate body lines.
  - Fix: `scripts/mamba3_make_source_stratified_splits.py` and `scripts/mamba3_make_splits.py` now write JSONL records when the output path ends in `.jsonl`.
  - New canonical split: `data/splits/base_doc_cont_v3_train.jsonl` and `data/splits/base_doc_cont_v3_valid.jsonl`.
...
- [x] Verify recovery on corrected v3 split.
  - Run: `STEPS=250 SAVE_EVERY=250 LR=1e-5 DATA_SEED=880001 scripts/mamba3_continue_clean_doc_base.sh`.
  ...
  - Artifacts: `runs/mamba3_clean_doc_base_moe900_v1/continuations/20260613T004652Z_*`.
...
- [ ] Continue clean raw-document pretraining until held-out doc loss is below 5.0 and raw continuations stop collapsing.
  - For maximum parameter count, continue `scripts/mamba3_continue_max_moe_base.sh`.
  - For governed repeated runs, use `scripts/mamba3_train_max_moe_until_gate.sh`; a decode with long repeated-word runs must never pass even if it produces many tokens.
  - Current 2.4B trajectory: held-out doc loss `11.6719 -> 10.3281 -> 9.7656 -> 9.5781 -> 9.5195`; still collapsed on greedy raw decode.
  - For faster iteration and more tokens/day, continue `scripts/mamba3_continue_clean_doc_base.sh` on the 900M or run the 1.3B throughput candidate after a clean from-scratch base run.
...
- [x] Add SISO -> MIMO r2 -> MIMO r4 stability ladder script.
  - `scripts/mamba3_train_stability_ladder.sh`
...
- [x] Add deterministic clean-English SFT generator.
  - `scripts/mamba3_generate_clean_english_sft.py`
...
- [x] Canonicalize the English-first top-intelligence data/post-training recipe without duplicating it across docs.
  - Single source of truth: `configs/mamba3_english_top_intelligence_recipe.json`.
  - Governance schema: `configs/mamba3_data_governance_schema.json`.
...
- [ ] Implement governed corpus expansion toward the canonical recipe.
  ...
  - Every produced JSONL must pass `scripts/mamba3_validate_governance.py`.
...
- [x] Add raw document-continuation corpus builder for early pretraining.
  - Script: `scripts/mamba3_build_doc_continuation_corpus.py`.
...
- [x] Wire document-continuation format into governed base expansion.
  - `scripts/mamba3_expand_governed_base.sh` now defaults to `DOC_CONTINUATION=1`.
...
- [x] Run first raw document-continuation canary on `ml-dmc8`.
  - Corpus: `data/base_doc_continuation_v1.jsonl`.
  - Records: 39,647 written, 353 short or instruction-like records skipped.
  - Split: `data/splits/base_doc_cont_train.txt` 38,856 records, `data/splits/base_doc_cont_valid.txt` 791 records.
  - Run: `runs/mamba3_doc_cont_v1/base_doc.pt`.
...
- [x] Add exact-match eval for the no-teacher curriculum.
  - `scripts/mamba3_eval_programmatic.py`
...
- [x] Run first base-first `mamba3-recall-r2-tiny` governed training pass.
  - Run: `runs/mamba3_recall_r2_base_first_v1`.
...
- [x] Add masked answer-only curriculum loss.
  ...
  - Reproducible script option: `CURRICULUM_LOSS=answer scripts/mamba3_train_base_first_recall.sh`.
...
- [x] Add interleaved base + masked-answer multitask training.
  - CLI: `train-multitask`.
  - Script path: `CURRICULUM_LOSS=multitask scripts/mamba3_train_base_first_recall.sh`.
...
- [x] Run first multitask comparison from the 800-step base checkpoint.
  - Run: `runs/mamba3_recall_r2_base_first_v1/curriculum_multitask.pt`.
...
- [x] Add hard-recall curriculum shard generation.
  - `scripts/mamba3_generate_programmatic_curriculum.py --tasks copy,phonebook_lookup,json_field_extraction,needle_in_haystack`.
...
- [x] Run hard-recall multitask comparison from the 800-step base checkpoint.
  - Run: `runs/mamba3_recall_r2_base_first_v1/curriculum_hard_multitask.pt`.
...
- [x] Add staged recall curriculum levels.
  - `scripts/mamba3_generate_programmatic_curriculum.py --difficulty easy`.
...
- [x] Run easy staged recall canary from the 800-step base checkpoint.
  - Run: `runs/mamba3_recall_r2_base_first_v1/curriculum_easy_multitask.pt`.
...
- [x] Add reproducible base-only continuation script.
  - `scripts/mamba3_continue_base_training.sh`.
...
- [x] Run first longer base-only continuation.
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_long.pt`.
...
- [x] Add reproducible governed base expansion script.
  - `scripts/mamba3_expand_governed_base.sh`.
  - Increases FineWeb-Edu/DCLM streaming budgets, regenerates a clean-English supplement, and writes `data/splits/base_expanded_train.txt` plus `data/splits/base_expanded_valid.txt`.
- [x] Run governed corpus expansion.
  - Command: `MAX_DOCS=10000 MAX_BYTES=250000000 CLEAN_RECORDS=4000 scripts/mamba3_expand_governed_base.sh`.
...
- [x] Run expanded base-only continuation.
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_expanded_long.pt`.
...
- [x] Add math/science/technical shards to the governed base expansion path.
  ...
  - `scripts/mamba3_generate_technical_bootstrap.py` creates deterministic no-teacher code/math/science/structured-data records.
...
- [x] Run math/science/technical base-only continuation before source cleanup.
  ...
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_tech_math_science_long.pt`.
...
- [x] Continue base-only training from `base_tech_math_science_long.pt` on the cleaned split.
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_clean_abstract_long.pt`.
...
- [ ] Reduce deterministic technical/structured-data shard weight or separate it from general base quality training.
  ...
  - Default `TECH_RECORDS` in `scripts/mamba3_expand_governed_base.sh` is reduced from 4000 to 1200 for the next base split.
...
- [x] Rebalance the cleaned split with lower deterministic technical weight.
  - Command: `MAX_DOCS=10000 MAX_BYTES=250000000 CLEAN_RECORDS=5000 TECH_RECORDS=1200 scripts/mamba3_expand_governed_base.sh`.
  - FineWeb-Edu: 10,000 documents, 47,357,610 text bytes.
  - DCLM: 10,000 documents, 55,873,154 text bytes.
...
- [x] Continue base-only training after rebalancing the cleaned split.
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_rebalanced_recovery.pt`.
...
- [x] Rebuild the base continuation path from an earlier clean checkpoint with `TECH_RECORDS=0`.
  - Command: `MAX_DOCS=10000 MAX_BYTES=250000000 CLEAN_RECORDS=5000 TECH_RECORDS=0 TRAIN_OUT=data/splits/base_general_recovery_train.txt VALID_OUT=data/splits/base_general_recovery_valid.txt scripts/mamba3_expand_governed_base.sh`.
  ...
  - Technical bootstrap was skipped and stale `data/technical_bootstrap_v1.jsonl` is removed by the script when `TECH_RECORDS=0`.
...
- [x] Run base-only recovery from the earlier clean checkpoint on the `TECH_RECORDS=0` split.
  - Run: `runs/mamba3_recall_r2_base_first_v1/base_general_recovery_from5600.pt`.
...
- [x] Add reproducible architecture/context comparison script.
  - Script: `scripts/mamba3_run_arch_context_compare.sh`.
...
- [x] Run the first controlled architecture/context comparison on `ml-dmc8`.
  - Run roots:
    - `runs/mamba3_arch_context_compare_quick_v1`
    - `runs/mamba3_arch_context_compare_quick_v1_r4`
...
- [x] Add source-stratified train/valid split generation.
  ...
  - Script: `scripts/mamba3_make_source_stratified_splits.py`.
  - `scripts/mamba3_expand_governed_base.sh` now uses the source-stratified splitter by default; set `STRATIFIED_SPLIT=0` to use the old global shuffle.
  - Generated on `ml-dmc8`:
    - `data/splits/base_general_recovery_strat_train.txt`: 44,100 records.
    - `data/splits/base_general_recovery_strat_valid.txt`: 900 records.
...
- [x] Add a small Transformer baseline under the same tokenizer/data/token budget.
  ...
  - CLI preset source was refactored into `mamba3_kr/presets.py` so CLI parsing no longer imports the heavy Mamba stack.
...
- [x] Make `mimo-r4-tiny` runtime parity-safe by default.
  - `./neurova.sh` and `scripts/mamba3_chat_serverctl.sh` default to `decode_mode=safe`.
  ...
  - Unsafe MIMO recurrent-cache decode remains research-only until `scripts/mamba3_recurrent_parity.py` passes without `--exact-cache`.
- [x] Restore official Mamba-3 recurrent promotion rules.
  ...
  - `scripts/mamba3_promote_if_pass.sh` now rejects `mimo-r4-tiny` as a fast recurrent runtime and runs recurrent parity before promotion.
- [x] Pivot 4080 mainline to Mamba-3 SISO hybrid.
  ...
  - Added `scripts/mamba3_train_siso_hybrid_v1.sh` and `scripts/mamba3_siso_hybrid_trainctl.sh`.
  - Started background run: `runs/mamba3_siso_hybrid_0_3b_v1`.
- [x] Warmup-excluded speed benchmark.
- [x] Add quality/speed decode sweep for the latest chat checkpoint.
  - Script: `scripts/mamba3_decode_tune.py`.
...
- [x] Add real-time streaming to `./neurova.sh`.
  ...
  - Current measured stream path on `runs/mamba3_current/model.pt`: about 18-25 tok/s on simple QA.
...
- [x] DeepSpeed ZeRO config files.
  - `configs/deepspeed_zero2_cpu_offload.json` for optimizer CPU offload.
  - `configs/deepspeed_zero3_param_offload.json` for final-resort parameter CPU offload.
...
- [ ] Promote recurrent/CUDA-graph chat only when `scripts/mamba3_decode_tune.py` selects a recurrent config with no quality regression and `scripts/mamba3_recurrent_parity.py` passes without `--exact-cache`.
...
- [x] Add context-limit probe script.
  - `scripts/mamba3_probe_context_limits.sh`
...
- Current fast run: `runs/mamba3_siso_fast_0_3b_ds128_v3`.
- Previous fast baseline run: `runs/mamba3_siso_fast_0_3b_v1`.

## Speed and Intelligence Research Update

Evidence from the fast SISO benchmark:

- `scripts/mamba3_speed_intel.py` measures recurrent decode throughput and separates trunk vs LM-head cost.
...
- [x] Add a clean multitask continued-pretraining line for `mamba3-siso-fast-0.3b-ds128`.
  - Script: `scripts/mamba3_train_siso_fast_0_3b_intel_v1.sh`.
  - Controller: `scripts/mamba3_siso_fast_intel_trainctl.sh`.
  - Run dir: `runs/mamba3_siso_fast_0_3b_ds128_intel_v1`.
  - Seed checkpoint: `runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt`.
...
- [ ] Start the 0.3B intelligence continuation run when GPU ownership should move from the 2.4B diagnostic block.
  - Command: `scripts/mamba3_siso_fast_intel_trainctl.sh start`.
  - It uses `scripts/mamba3_exclusive_gpu_guard.sh`, so the 2.4B autopilot/watchdog/training line is stopped during the 0.3B run and restored afterwards.
...
- [x] Add a no-cheat knowledge corpus builder for real intelligence improvement.
  - Script: `scripts/mamba3_build_no_cheat_knowledge_corpus.py`.
...
- [x] Add a no-cheat 0.3B knowledge-pretraining line.
  - Script: `scripts/mamba3_train_0_3b_no_cheat_knowledge_v1.sh`.
  - Controller: `scripts/mamba3_no_cheat_knowledge_trainctl.sh`.
  - Run dir: `runs/mamba3_siso_fast_0_3b_ds128_no_cheat_knowledge_v1`.
  - Seed checkpoint: stable `runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt`, not the degraded `intel_v1` checkpoint.
...
- Programmatic state-memory evaluator fixes:
  - `scripts/mamba3_eval_programmatic.py` now supports answer markers `Answer:`, `Assistant:`, and `A:`.
...
- Task-filtered curriculum generation:
  - `scripts/mamba3_generate_state_memory_curriculum.py --tasks ...`
...
- `runs/mamba3_siso_fast_0_3b_ds128_v4_copy/model.pt`
...
- `runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt` is the current default `./neurova.sh` checkpoint.
...
- `START_CHECKPOINT` is supported in `scripts/mamba3_train_siso_hybrid_v1.sh`, so new candidates can seed from a previous checkpoint without overwriting it.
- `./neurova.sh` now defaults to `runs/mamba3_siso_fast_0_3b_ds128_v3/model.pt`, with v1 as fallback.
...
- `runs/mamba3_siso_fast_0_3b_ds128_v2` fixed the "do not know" prompt but still answered `Repeat the word yes forever.` as `No. I should`; it is preserved but not promoted.
- `runs/mamba3_siso_fast_0_3b_ds128_v4_copy` improved phonebook exact match but did not solve copy/routing and is not promoted.
...
- Re-tested `scripts/mamba3_state_compile_bench.py` on `runs/mamba3_current_training_chat/model.pt`.
...
- `scripts/mamba3_autonomous_hybrid_research_loop.sh`
...
- `scripts/mamba3_autonomous_hybrid_researchctl.sh`
...
- `runs/mamba3_current_training/model.pt`
...
- `runs/mamba3_current_training_chat/model.pt`
...
Goal: raise intelligence without breaking the fast pure-SISO recurrent serving path. These candidates do not replace `runs/mamba3_current/model.pt` unless they beat the current model on held-out eval and chat gates.

- [x] Expose Mamba-3 `is_outproj_norm` through `Mamba3KRConfig`.
...
- [x] Add autonomous ablation loop `scripts/mamba3_siso_intel_ablation_loop.sh`.
